...
def build_cnn(seq_length):
  ...
  model = tf.keras.Sequential([
      tf.keras.layers.Conv2D(8, (4, 3), padding="same", activation="relu", input_shape=(seq_length, 3, 1)),  # output_shape=(batch, 128, 3, 8)
...
def train_net(
  ...
  open("lastFirst.tflite", "wb").write(tflite_model)
  ...
  # Save the model to disk
  open("lastFirst_quantized.tflite", "wb").write(tflite_model)
  ...
  basic_model_size = os.path.getsize("lastFirst.tflite")
  print("Basic model is %d bytes" % basic_model_size)
  quantized_model_size = os.path.getsize("lastFirst_quantized.tflite")
